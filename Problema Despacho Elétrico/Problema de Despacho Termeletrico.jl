...
@constraint(model, [t = 1:T, s=1:S] , gen_tot[t, s] + deficit[t,2, s] + flow_se_s[t, s] .== demand[2][t] + flow_s_se[t, s])

@constraint(model, max_flow_se[t = 1:T, s=1:S]          , flow_se_s[t, s] <= F)
@constraint(model, max_flow_s[t = 1:T, s=1:S]           , flow_s_se[t, s] <= F)
@constraint(model, limite_cap[t = 1:T, i = 1:nT, s=1:S] , thermo_gen[t,i, s] <= capacity[i][t])
@constraint(model, max_variacao[t = 2:T, s=1:S]         , thermo_gen[t,3, s] - thermo_gen[t-1,3, s] <= 1.)
...
@constraint(model, generation[t=1:T, s=1:S]             , sum(thermo_gen[t,:, s]) + sum(wind_gen[t,:, s]) + sum(hidr_gen[t,:, s]) == gen_tot[t, s])
